COMPLETED: Implemented BSP tree construction including recursive splitting, face extraction, T-junction fixing, tree flattening, leaf cluster assignment and edge deduplication. Created `BspCompiler` integrating all stages. WASM verification deferred.
...
WASM verification requires comparing the output generated by the TypeScript BspCompiler with the original q2tools C implementation compiled to WebAssembly. This involves:
- Integrating the Emscripten toolchain to compile q2tools logic.
- Adding a test infrastructure capable of executing the WASM module.
- Running the generated map definitions through both the TS compiler and WASM logic to assert that internal representations match.

*See Section 25-9 for details on pending separate work items related to WASM infrastructure integration.*

### 11.1 Structure Comparison

- [ ] Compare node/leaf counts between TS and WASM output.
- [ ] Compare tree depth and balancing parameters.
- [ ] Compare face counts, edge counts, and plane generation counts.

### 11.2 Semantic Comparison

- [ ] Point queries return same leaf structures between implementations.
- [ ] Trace queries return identical structural intersections.
