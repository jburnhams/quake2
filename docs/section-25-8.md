...
- [ ] Test: Engine renders lightmaps correctly (Deferred to separate integration testing work)

---

## 11. WASM Verification (Deferred to Separate Work Items)

WASM verification for lightmaps involves creating a separate test infrastructure to run the original C implementation of `q2tools` as WebAssembly and executing lightmap generation algorithms side-by-side with the TypeScript `computeRadiosity` and direct lighting implementations.

*See Section 25-9 for details on pending separate work items related to WASM and Emscripten.*

### 11.1 Lightmap Comparison

- [ ] Compare lightmap dimensions per face generated by WASM vs TS.
- [ ] Compare average brightness per face within floating-point tolerance.
- [ ] Compare total packed lighting data sizes to detect anomalies.

### 11.2 Visual Comparison

- [ ] Using the Quake 2 WebGL engine, render identical viewpoints in a test map using both BSP formats.
- [ ] Develop automated screenshot comparison tools to flag significant discrepancies in radiosity bounds or tone mapping logic.
