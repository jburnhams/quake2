...
COMPLETED: Evaluated and updated verification checklist for lightmaps. Implemented light parsing, direct lighting computation, radiosity patches, lightmap sizing, lightmap packing and multiple styles, integrated with BspCompiler.
...
1. **WASM Verification for Lightmaps:** Compare lighting data outputs, average brightness, lightmap dimensions, and total size generated by `bsp-tools` with the original `q2tools` output via WASM.
2. **Visual Engine Verification via WASM:** Setup visual comparisons of screenshots of the rendered BSP using the typescript generated data vs WASM-generated data.
