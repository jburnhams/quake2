...
- [ ] Output matches WASM reference within floating-point tolerance (Deferred)
- [ ] Generated BSPs load in quake2ts engine (Deferred)
- [ ] Performance acceptable for real-time procedural generation of simple maps (Deferred)

### Pending Separate Work Items

The following testing and verification criteria require infrastructure that falls outside the immediate scope of building out the BSP toolchain implementation:
...
2. **Engine rendering tests:** Building visual integration tests and interactive pipelines to ensure `quake2ts` accurately renders the BSP generated by `@quake2ts/bsp-tools`.
3. **Performance profiling:** Executing formal `vitest` benchmarks on large compilation targets to profile and optimize the overall process.
