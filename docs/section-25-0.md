...
COMPLETED: Implementation of the BSP toolchain is well underway. Foundations, winding math, map parsing, primitive building, CSG, BSP tree construction (including T-junction fixing), visibility generation (portals, PHS, PVS), and advanced lighting (parsing, tracing, radiosity, lightmaps) are now complete. All document sections have been implemented or marked deferred for separate engine/infrastructure tasks.
