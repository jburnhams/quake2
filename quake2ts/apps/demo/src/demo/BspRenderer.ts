import { BspLoader, BspMap, BspSurfaceInput, buildBspGeometry, BspSurfaceGeometry, LightmapAtlas, gatherVisibleFaces, BspSurfacePipeline, applySurfaceState, TextureCache, AssetManager, VirtualFileSystem, BspFace, BspTexInfo, Texture2D, PreparedTexture } from '@quake2ts/engine';
import { Camera } from '@quake2ts/engine';
import { Mat4, aabb, Vec3 } from '@quake2ts/shared';

function extractBspSurfaces(map: BspMap, textureCache: Map<string, PreparedTexture>): BspSurfaceInput[] {
    ...
    for (let i = 0; i < map.faces.length; i++) {
        ...
        for (let j = 0; j < face.numEdges; j++) {
            ...
            const vertex = map.vertices[edgeIndex > 0 ? edge.vertices[0] : edge.vertices[1]];
...
export class BspRenderer {
    ...
    render(camera: Camera, map: BspMap) {
        ...
        const modelViewProjection = Mat4.multiply(projectionMatrix, viewMatrix);
