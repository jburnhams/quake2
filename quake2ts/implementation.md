...
6. **Shared pmove module**: Port rerelease movement to TypeScript in `packages/shared`; validate with golden tests for known edge cases (stairs, slides, water, air control quirks). The shared helpers now include a pure `applyPmoveAirAccelerate` that mirrors `PM_AirAccelerate` from `rerelease/p_move.cpp` (lines 612-636), and the color blend helper matches `G_AddBlend` from `rerelease/q_std.h` (line 151) so HUD/effects color math remains faithful.
