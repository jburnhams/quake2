...
import { T_RadiusDamage, Damageable, EntityDamageFlags } from '../../src/combat/damage.js';
import { DamageFlags } from '../../src/combat/damageFlags.js';
import { DamageMod } from '../../src/combat/damageMods.js';
import { Vec3 } from '@quake2ts/shared';

describe('T_RadiusDamage', () => {
  const createMockEntity = (overrides: Partial<Damageable> = {}): Damageable => ({
    takedamage: true,
    health: 100,
    origin: { x: 0, y: 0, z: 0 },
  ...
  it('should apply damage to entities within radius', () => {
    const victim = createMockEntity({ origin: { x: 50, y: 0, z: 0 } });
    const attacker = createMockEntity({ origin: { x: 200, y: 200, z: 200 } });
  ...
  it('should apply half damage to attacker (self-damage)', () => {
    ...
    const attacker = createMockEntity({ origin: { x: 10, y: 0, z: 0 } });
    const inflictor = { origin: { x: 0, y: 0, z: 0 } };
