...
import { createEntityFactory, createPlayerEntityFactory, createPlayerClientFactory, createTestContext, spawnEntity } from '@quake2ts/test-utils';
...
describe('T_RadiusDamage', () => {
  ...
  it('applies linear falloff and halves self damage', () => {
    const inflictor = spawnEntity(context, createEntityFactory({ origin: { x: 0, y: 0, z: 0 } }));
...
describe('Damage Modifiers', () => {
    ...
    beforeEach(() => {
        ...
        if (!attacker.client) {
            attacker.client = createPlayerClientFactory({ quad_time: 0, double_time: 0 });
        } else {
    ...
    it('applies quad damage modifier', () => {
        attacker.client!.quad_time = time + 5;
        const result = T_Damage(target, attacker, attacker, { x: 1, y: 0, z: 0 }, target.origin, { x: 0, y: 0, z: 1 }, damage, knockback, DamageFlags.NONE, MOD_UNKNOWN, time);
        expect(result?.take).toBe(damage * 4);
    });

    it('applies double damage modifier', () => {
        attacker.client!.double_time = time + 5;
        const result = T_Damage(target, attacker, attacker, { x: 1, y: 0, z: 0 }, target.origin, { x: 0, y: 0, z: 1 }, damage, knockback, DamageFlags.NONE, MOD_UNKNOWN, time);
