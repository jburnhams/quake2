...
import { createEntityFactory, spawnEntity } from '@quake2ts/test-utils';
...
describe('func_door', () => {
  ...
  it('should open on touch if no health or targetname', () => {
    ...
    // Mock other entity
    const other = spawnEntity(context.entities, createEntityFactory({ number: 2, classname: 'player' }));
  ...
  it('should inflict damage and reverse when blocked', () => {
    ...
    const victim = spawnEntity(context.entities, createEntityFactory({ number: 2, takedamage: true, health: 100 }));
