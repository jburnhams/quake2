...
describe('monster_makron', () => {
  ...
  it('should spawn with correct properties', () => {
    SP_monster_makron(self, context);

    expect(self.classname).toBe('monster_makron');
    ...
    expect(self.mins).toEqual({ x: -30, y: -30, z: 0 });
    expect(self.maxs).toEqual({ x: 30, y: 30, z: 90 });
  });

  it('should set initial state to sight', () => {
    SP_monster_makron(self, context);
    expect(self.monsterinfo.current_move).toBeDefined();
    // Sight move is 13 frames
    expect(self.monsterinfo.current_move?.frames.length).toBe(13);
  });
});
