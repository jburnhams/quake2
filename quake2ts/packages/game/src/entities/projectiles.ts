...
import { ZERO_VEC3, lengthVec3, subtractVec3, normalizeVec3, Vec3, CollisionPlane, ServerCommand, TempEntity, addVec3, scaleVec3 } from '@quake2ts/shared';
...
// See g_weapon.c, bfg_think
function bfg_think(self: Entity, sys: EntitySystem) {
    ...
    for (const target of targets) {
        ...
        const losTrace = sys.trace(self.origin, null, null, target.origin, self, 0x10020002);
        if (losTrace.fraction < 1.0 && losTrace.ent !== target) {
            continue; // Blocked
        }

        // Fire a piercing laser beam towards the target.
        // See: rerelease/src/game/g_weapon.cpp: bfg_think
        const beamlist: Entity[] = [];
        ...
        let ignore: Entity | null = self;

        for (let i = 0; i < 16; i++) { // Safety break
            const trace = sys.trace(currentStart, null, null, end, ignore, 0x10020002);

            if (trace.ent && trace.ent.takedamage) {
                // Don't hit the same entity twice in the same beam.
                if (!beamlist.includes(trace.ent)) {
                    beamlist.push(trace.ent);
                    T_Damage(
                        trace.ent as any, self as any, self.owner as any, dir, trace.endpos,
                        trace.plane ? trace.plane.normal : ZERO_VEC3,
                        10, 1, DamageFlags.ENERGY, DamageMod.BFG_LASER,
                        sys.multicast.bind(sys)
                    );
                }
            }

            if (trace.fraction >= 1.0 || (trace.ent && trace.ent === sys.world)) {
                // Reached end of trace or hit world
                sys.multicast(self.origin, MulticastType.Pvs, ServerCommand.temp_entity, TempEntity.BFG_LASER, self.origin, trace.endpos);
                break;
            }
            ...
            if (trace.ent) {
                ...
                // Also need to draw the beam if it hit nothing and ended early
                sys.multicast(self.origin, MulticastType.Pvs, ServerCommand.temp_entity, TempEntity.BFG_LASER, self.origin, trace.endpos);
                break;
            }
        }
...
// See g_weapon.c, bfg_explode
function bfg_explode(self: Entity, sys: EntitySystem) {
    if (!self.owner) {
        return;
    }

    // First frame of explosion deals radius damage.
    // Subsequent frames just spawn visual effects.
    if (self.count === 0) {
        ...
        for (const target of targets) {
            if (target !== self.owner && target.takedamage) {
                ...
                if (trace.fraction >= 1.0 || trace.ent === target) {
                    ...
                    const points = 100 * (1.0 - Math.sqrt(dist / 1000));
                    if (points > 0) {
                        ...
                        T_Damage(
                            ...
                            Math.floor(points), 1, DamageFlags.ENERGY, DamageMod.BFG_EFFECT, sys.multicast.bind(sys)
                        );
                    }
                }
            }
...
// See g_weapon.c, bfg_touch
function bfg_touch(self: Entity, other: Entity | null, plane: CollisionPlane | null | undefined, surf: any, sys: EntitySystem) {
    if (!self.owner) {
        return;
    }
    ...
    T_RadiusDamage(entities as any[], self as any, self.owner as any, 200, self.owner as any, 1000, DamageFlags.NONE, DamageMod.BFG_BLAST, {}, sys.multicast.bind(sys));
    ...
    self.think = (self: Entity) => bfg_explode(self, sys);
    self.count = 0;
    sys.scheduleThink(self, sys.timeSeconds + 0.1);
}
...
export function createBfgBall(sys: EntitySystem, owner: Entity, start: Vec3, dir: Vec3, damage: number, speed: number, damageRadius: number): Entity {
    ...
    bfgBall.touch = (self: Entity, other: Entity | null, plane?: CollisionPlane | null, surf?: any) => bfg_touch(self, other, plane, surf, sys);
    bfgBall.think = (self: Entity) => bfg_think(self, sys);
