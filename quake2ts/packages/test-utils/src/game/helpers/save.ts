...
export function createSaveGameSnapshot(context: TestContext): MockSaveGame {
    ...
    const game = context.game as { level?: { frameNumber?: number, timeSeconds?: number }, time?: number };
    const levelFrameState: LevelFrameState = {
        frameNumber: game.level?.frameNumber ?? 0,
        timeSeconds: game.level?.timeSeconds ?? game.time ?? 0,
        previousTimeSeconds: 0,
        deltaSeconds: 0.1
    };

    const currentLevel = ('level' in context.entities ? (context.entities as unknown as { level: LevelState }).level : {}) as LevelState;
...
export function restoreSaveGameSnapshot(saveGame: MockSaveGame, context: TestContext): void {
    ...
    // Restore level state
    if ('level' in context.entities) {
        Object.assign((context.entities as unknown as { level: LevelState }).level, saveGame.level);
    }
}
