...
2. ✅ Move pure logic tests in shared package to `tests/unit-node/` directories (vec3, mat4, angles, contents, color, constants)
   ...
   - Moved: `engine/tests/unit-jsdom/render/context.test.ts` to `engine/tests/unit-node/render/context.test.ts`
   - Added: `engine/tests/unit-node/render/bspPipelineHelpers.test.ts`
