...
- [x] Touch detection
  - [x] When entities overlap (via trace), call touch callbacks
  - [x] Handle trigger volumes
  - [x] Pickup items, teleporters, hurt triggers
  - **Note**: Implementation updated to use `CollisionEntityIndex` for O(1) average case spatial queries in `runTouches` and `killBox`, replacing previous O(N^2) brute-force iteration.
...
- [x] Touch detection
  - [x] After movement, check for entity overlaps
  - [x] Call touch callbacks on both entities
  - [x] Handle pickup, trigger, damage logic
  - **Note**: Optimized with `CollisionEntityIndex`.
