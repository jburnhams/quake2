# Section 15.1: Missing Weapons

**Priority**: P1 (base) / P3 (mission packs) | **Effort**: 3 weeks | **Parallel**: Yes

## Base Game

### Hand Grenade (Throwable)
- **Source**: `../rerelease/p_weapon.cpp:988-1213`
- **Priority**: HIGH - base game weapon
- [ ] Implement hold-to-cook timer (max 3 sec)
- [ ] Variable throw speed: 400-800 based on hold time
- [ ] In-hand explosion if held too long
- [ ] Throw animation (FRAME_THROW_HOLD, FRAME_THROW_FIRE)
- [ ] Prime sound ("weapons/hgrent1a.wav") + primed loop
- [ ] Test: Throw at various hold times, verify speed variation

## Ground Zero (Rogue) - 6 Weapons

### Plasma Beam
- **Source**: `../rogue/p_rogue_weapon.cpp`
- [ ] Continuous beam (like lightning gun)
- [ ] Cell consumption per frame
- [ ] Visual beam effect
- [ ] Pushback on player

### Ion Ripper
- **Source**: `../rogue/g_rogue_weapon.cpp`
- [ ] Spinning blade projectile
- [ ] Bounces off walls (max 5 bounces)
- [ ] Damage on each contact
- [ ] Ricochet sound

### ETF Rifle
- **Source**: `../rogue/g_rogue_weapon.cpp`
- [ ] Freeze ray
- [ ] Frozen enemies shatter on hit
- [ ] Ice visual effect
- [ ] Temperature system

### Phalanx
- **Source**: `../rogue/g_rogue_weapon.cpp`
- [ ] Large plasma ball
- [ ] Massive radius damage
- [ ] Purple energy effect

### Prox Mine Launcher
- **Source**: `../rogue/g_rogue_weapon.cpp`
- [ ] Fires sticky mines
- [ ] Proximity trigger (enemy detection)
- [ ] Laser tripwire beam
- [ ] Team-aware (doesn't trigger for owner)
- [ ] Max 50 mines per player

### Chainfist
- **Source**: `../rogue/g_rogue_weapon.cpp`
- [ ] Chainsaw-style melee
- [ ] High close-range damage
- [ ] Continuous damage while held
- [ ] Gore effects

## Xatrix (The Reckoning) - 1 Weapon

### Trap
- **Source**: `../xatrix/g_xatrix_weapon.cpp`
- [ ] Deployable laser trap
- [ ] Damages enemies crossing beam
- [ ] Limited lifetime
- [ ] Visual laser effect

## Implementation Notes
- Hand grenade is only P1 (base game)
- Mission pack weapons are P3 (optional)
- Each weapon can be done independently
- Hand grenade estimated: 1 week
- Each mission pack weapon: 2-3 days
