...
- [x] Weapon spin-up/down sounds - *Confirmed implemented in Gunner (gunatck1.wav), additional verification in test `gunner_sound.test.ts`*
...
- [x] Burn gibs (fire deaths) - *Implemented: prevents blood trails when damage mod is LAVA or TRAP*
...
- **Priority**: P2
- **Effort**: 3 days
- [x] Medic Commander spawns reinforcements - *Implemented in `medic.ts` via `medic_call_reinforcements` logic.*
...
- [x] Used by Tank Commander, Supertank - *Integrated into `supertank.ts` and `tank.ts` via spawnflags.*
...
- [x] trigger_flashlight control - *Implemented in `game` entities*
