...
- [x] **Weapon_Repeating() - Auto Weapons**
...
- [x] **Instant Weapon Switch Option**
...
- [x] **Animation State (add to client)**
...
- [x] **Hand Grenade Throw Animations**
...
- [x] Player model animates while moving
- [x] Hand grenade throw shows proper body animation
- [x] Crouching vs standing throws use different animations
- [x] Pain animations play when damaged
- [x] Death animations play on death
...
- [x] All weapons have raise/lower/fire/idle animations (Infrastructure ready, Hand Grenade implemented, Shotgun implemented, Super Shotgun implemented, Machinegun implemented, Chaingun implemented, Blaster, Rocket Launcher, Railgun, Hyperblaster, Grenade Launcher, BFG10K implemented and tested)
...
- [x] Player body shows throw animation when throwing grenade
- [x] Different animations for ducking vs standing
- [x] All combat actions have matching body animations
...
- None. All weapons migrated and verified.

**Estimate**: Hand Grenade implementation is 100% complete. Weapon system infrastructure is in place and verified for all weapons.

## Refactoring Notes (Completed)
- Removed legacy fallback code in `firing.ts` that allowed instant firing without animation frames.
- Updated unit tests to strictly adhere to animation frame sequences.
- Fixed `Weapon_Repeating` signature to support pause frames in idle state.
- Resolved circular dependency between `animation.ts` and `firing.ts` by extracting `random` generator.
- Added logic to `Weapon_Repeating` to advance `weapon_think_time` to prevent double frame advancement.
