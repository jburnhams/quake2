...
- [x] **Define Protocol Types:**
    ...
    - Implemented in `packages/shared/src/protocol/ops.ts` (Command enums).
    - `packages/server/src/protocol/write.ts` handles server command serialization.
    - `packages/server/src/protocol.ts` (ClientMessageParser) handles client command parsing.
    - Ensure byte-perfect alignment with the original protocol where possible.
- [x] **Message Parser/Writer:**
    - Implement a `NetMessage` class to read/write binary data.
    ...
    - Unit tests exist in `packages/server/tests/protocol/write.test.ts`.
...
- [ ] **Local Loopback Transport:**
    ...
    - Future refactoring may be needed to decouple `packages/server` logic (dependent on `ws`) for browser-based loopback, or verify if `packages/server` can run in a browser context (it seems to have node dependencies).
...
- **Existing Implementation:** Substantial parts of the protocol serialization (server-side) exist in `packages/server`, specifically `writeDeltaEntity` and `writeServerCommand`. This package should be the primary reference for protocol implementation, though dependency management (Node vs Browser) needs consideration for local play.
