...
- [x] Extract to constants with source refs (Started with `boss2.ts`, `jorg.ts`, `infantry.ts`, `gunner.ts`, `berserk.ts`, `parasite.ts`)
```typescript
...
- [x] Enable `strictNullChecks` in tsconfig (Dry Run Completed: 0 errors found - enabled by default in base config)
...
- [x] Find all `: any` (Cleaned up `entity.ts`, `prox.ts`, `projectiles.ts`, `collision.ts`, `gibs.ts`, `entities/misc/*`, `entities/monsters/jorg.ts`)
...
- [x] Extract magic numbers to constants (Completed for `boss2.ts`, `jorg.ts`, `infantry.ts`, `gunner.ts`, `berserk.ts`, `parasite.ts`)
- [ ] Enable strictNullChecks and fix
- [x] Replace any types (Collision system surface passing, projectiles, prox mine, gibs, entities/misc/*, entities/monsters/jorg.ts)
