...
- [x] Extract to constants with source refs (Started with `boss2.ts`, `jorg.ts`)
```typescript
...
- [x] Find all `: any` (Started cleanup in `entity.ts` and `prox.ts`)
...
- [x] Extract magic numbers to constants (Completed for `boss2.ts`, `jorg.ts`)
- [ ] Enable strictNullChecks and fix
- [ ] Replace any types (Partial progress: `entity.ts`, `prox.ts`)
