# Section 16.2: Bad Comments

**Priority**: P2 | **Effort**: 1-2 days | **Parallel**: Yes

## Issues Found

### 1. Rambling Comments
**File**: `packages/game/src/physics/movement.ts:21-94`
- [ ] 50+ lines of stream-of-consciousness debugging
- [ ] Replace with concise doc comment
- [ ] Extract WATER_FRICTION = 2.0, WATER_GRAVITY_SCALE = 0.1 constants
- [ ] Document deviation from original clearly
- **Source**: `../rerelease/g_phys.cpp:145-160` G_RunObject

### 2. Contradictory Comments
**Files**: `parasite.ts:41-42`, `mutant.ts:41`, `chick.ts:42`, `flipper.ts:31`, `brain.ts:33`
```typescript
// Helper to access deterministic RNG or Math.random
const random = () => Math.random();  // Says deterministic, does Math.random
```
- [ ] Delete all these helpers
- [ ] Use `game.random.frandom()` directly
- [ ] See 16.1 for full fix

### 3. Uncertain Comments
**File**: `monsters/supertank.ts:313`
```typescript
self.viewheight = 64; // Guess, maybe higher?
```
- [ ] Look up correct value in `../rerelease/m_supertank.cpp`
- [ ] Replace with verified value + source ref
- [ ] Remove "guess", "maybe", "probably" language

**Find all**:
```bash
grep -rn "\(guess\|maybe.*?\|probably\|I think\|not sure\)" packages/ --include="*.ts" | grep "//"
```

### 4. Confused Test Comments
**File**: `shared/tests/bsp/trace.debug.test.ts:100,116,133`
```typescript
// Wait, why did it fail with 0?
// Maybe `traceBox` isn't using the updated collision.ts?
// Why is it 0?
```
- [ ] Investigate actual behavior
- [ ] Replace with explanation or delete test
- [ ] No "wait", "why", "maybe" in committed tests

### 5. Missing Comments on Complex Code
**File**: `shared/src/bsp/collision.ts:300-348`
- [ ] Add algorithm overview (Liang-Barsky clipping)
- [ ] Explain d1, d2, enterfrac, leavefrac
- [ ] Document epsilon usage
- [ ] Add source ref: `../rerelease/qcommon/cm_trace.c:145-220`

**Find all complex code**:
```bash
# Functions >30 lines with no comments
```

### 6. Obvious Comments
```typescript
i++;  // Increment i  ← DELETE
x = 5;  // Set x to 5  ← DELETE
```
- [ ] Delete or replace with WHY not WHAT

## Tasks
- [ ] Fix rambling comments (focus on movement.ts)
- [ ] Delete contradictory comments (covered in 16.1)
- [ ] Look up all guessed values
- [ ] Investigate or delete confused test comments
- [ ] Add comments to complex algorithms
- [ ] Delete obvious comments

## Search Commands
```bash
# Find uncertain language
grep -rn "guess\|maybe.*?\|probably" packages/ --include="*.ts"

# Find confused tests
grep -rn "wait.*why\|why.*?" packages/tests/ --include="*.ts"

# Find TODO/FIXME
grep -rn "TODO\|FIXME\|HACK" packages/ --include="*.ts"
```

## Success Criteria
- [ ] Zero "guess", "maybe", "probably" in comments
- [ ] Zero contradictory comments
- [ ] All complex algorithms (>30 lines) have explanatory comments
- [ ] All ported code has source reference
- [ ] Tests explain behavior, not confusion
