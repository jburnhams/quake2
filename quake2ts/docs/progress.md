...
- Added a `GameFrameLoop` that mirrors the rerelease `G_PrepFrame`/`G_RunFrame` timing (see `rerelease/g_main.cpp` lines 818-1017) by bumping `level.time` before each world tick and sequencing prep/sim/post hooks, plus unit coverage that exercises the stage ordering and accumulator math and a gravity integration smoke test in `createGame`.
- Extended that frame loop with stage registration/disposal so multiple systems can subscribe to prep/sim/post hooks deterministically, and introduced a `LevelClock` helper that mirrors the rerelease `level.time` bookkeeping so snapshots now surface `frameNumber`/`timeSeconds` alongside gravity/origin/velocity.
- Hardened the frame loop's stage removal by deferring compaction and skipping undefined slots so handlers can unsubscribe mid-stage without blowing up the iterator or skipping the remaining systems, keeping per-frame overhead minimal and adding unit coverage for the hot path.
