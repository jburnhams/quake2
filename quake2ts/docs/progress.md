...
- Mirrored the ground categorization half of `PM_CatagorizePosition` as a pure `categorizePosition` helper so both pmove callers can reuse the same slope/wall logic before stepping, complete with Vitest cases for steep slopes, wedged-against-wall recovery, fast upward motion, and watertype propagation via the shared `getWaterLevel` probe.
